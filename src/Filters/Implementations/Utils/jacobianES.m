...
function H = jacobianES(q, v, form)
	...
	switch form
		...
		case 'long'
			...
			H = [ 	2*qw*vx - 2*qy*vz + 2*qz*vy, 2*qx*vx + 2*qy*vy + 2*qz*vz, 2*qx*vy - 2*qw*vz - 2*qy*vx, 2*qw*vy + 2*qx*vz - 2*qz*vx;
			...
			% u = q(2:4);
			% t = skew(u) * x - q1*x;
			% H = 2 * [-t u*x * eye(3) - skew(t)]; 

		case 'short'
			H = [	2*qz*vy - 2*qy*vz,           2*qy*vy + 2*qz*vz, 2*qx*vy - 2*qw*vz - 4*qy*vx, 2*qw*vy + 2*qx*vz - 4*qz*vx ;
