...
function [ellipsoid_center, ellipsoid_transform] = findMagnetometerCalibrationByBartz(magnetometerRaw, showDetails)

	if ~exist('showDetails')
		showDetails = false;
	end
	...
	[e_center, e_radii, e_eigenvecs, e_algebraic] = ellipsoidFit([x y z]);
	...
	ellipsoid_center = e_center';
	ellipsoid_transform = real(comp);
